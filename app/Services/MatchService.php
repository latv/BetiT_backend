...
class MatchService
{
    ...
    public function simulateMatch($matchId)
    {
        ...
        if ($match->is_match_finished) {
            throw new Exception("Match has already finished");
        }

        $bets = $match->bets;
        
        $match->is_match_finished = true;
        $match->result = $this->calculateMatchResult($match);

        $match->save();

        foreach ($bets as $bet) {
            $bet->is_succesfull = $bet->result == $match->result;
            $bet->save();

            if ($bet->is_succesfull) {
    ...
    {
        $matches = Match::where('is_match_finished', false)
        ->orderBy('time')
        ->get();
    }
    }
    ...
    {
        try {
            $match = Match::where('is_match_finished', false)->orderBy('time')->firstOrFail();
        } catch (Exception $e) {
